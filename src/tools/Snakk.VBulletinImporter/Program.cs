...
// ─── Step 1: Community ──────────────────────────────────────────────────────
Console.WriteLine("[1/15] Creating community 'freakforum'...");
...
// ─── Step 2: Users ──────────────────────────────────────────────────────────
Console.WriteLine("[2/15] Importing users...");
sw.Restart();
...
// ─── Step 3: User Avatars ────────────────────────────────────────────────────
var avatarInputPath = config["AvatarInputPath"] ?? "";
var storageBasePath = config["FileStorage:BasePath"] ?? "";
var avatarCount = 0;
if (!string.IsNullOrEmpty(avatarInputPath) && Directory.Exists(avatarInputPath) && !string.IsNullOrEmpty(storageBasePath))
{
    Console.WriteLine("[3/15] Importing user avatars...");
    sw.Restart();
    avatarCount = await writer.WriteUserAvatarsAsync(avatarInputPath, storageBasePath);
    Console.WriteLine($"\n  Imported {avatarCount:N0} avatars ({sw.Elapsed.TotalSeconds:F1}s)\n");
}
else
{
    Console.WriteLine("[3/15] Skipping avatar import (AvatarInputPath not configured or directory not found)\n");
}

// ─── Step 4: User Roles ─────────────────────────────────────────────────────
Console.WriteLine("[4/15] Assigning user roles (Administrators / Super Moderators / Moderators)...");
sw.Restart();
await writer.WriteUserRolesAsync(vbUsers, userMapping, communityId);
Console.WriteLine($"  Done ({sw.Elapsed.TotalSeconds:F1}s)\n");

// ─── Step 5: User Social Links ──────────────────────────────────────────────
Console.WriteLine("[5/15] Importing social links (homepage / skype / icq)...");
sw.Restart();
await writer.WriteUserSocialLinksAsync(vbUsers, userMapping);
Console.WriteLine($"  Done ({sw.Elapsed.TotalSeconds:F1}s)\n");

// ─── Step 6: Hubs (vBulletin categories) ────────────────────────────────────
Console.WriteLine("[6/15] Importing hubs (vBulletin categories)...");
...
Console.WriteLine("[8/15] Importing discussions (vBulletin threads)...");
sw.Restart();
...
Console.WriteLine("[9/15] Importing discussion type extensions (link + poll metadata)...");
sw.Restart();
...
// ─── Step 10: Poll Votes ──────────────────────────────────────────────────────
Console.WriteLine("[10/15] Importing poll votes...");
sw.Restart();
...
// ─── Step 11: Posts ──────────────────────────────────────────────────────────
Console.WriteLine("[11/15] Importing posts (vBulletin posts)...");
sw.Restart();
...
// ─── Step 12: Reactions (positive post votes → Agree) ────────────────────────
Console.WriteLine("[12/15] Importing reactions (post votes → Agree)...");
sw.Restart();
var totalVotes = 0;
batchNum = 0;
...
// ─── Step 13: Thread Follows (subscribethread → Discussion follows) ───────────
Console.WriteLine("[13/15] Importing thread follows (subscribethread)...");
sw.Restart();
...
// ─── Step 14: User Follows (userlist buddies → User follows) ─────────────────
Console.WriteLine("[14/15] Importing user follows (buddy list)...");
sw.Restart();
...
// ─── Step 15: Denormalized counts ────────────────────────────────────────────
Console.WriteLine("[15/15] Updating denormalized counts...");
sw.Restart();
...
Console.WriteLine($"  Avatars:        {avatarCount:N0}");
